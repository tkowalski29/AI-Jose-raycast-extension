...
import { ConfigurationModelCollection, ConfigurationModelDefault, ConfigurationTypeCommunication, ConfigurationTypeCommunicationDefault } from "../../type/config";
import { TalkAssistantType } from "../../type/talk";

export const AssistantForm = (props: { assistant?: TalkAssistantType; use: { assistants: AssistantHookType }; name?: string }) => {
  const { use, assistant } = props;
  const { pop } = useNavigation();

  const { handleSubmit, itemProps } = useForm<TalkAssistantType>({
    onSubmit: async (assistant) => {
      let updatedItem: TalkAssistantType = { ...assistant };
      ...
      >
        <Form.Dropdown title="Temperature" placeholder="Temperature model" {...itemProps.modelTemperature}>
          {Array.from({ length: 10 }, (_, index) => (
            <Form.Dropdown.Item value={`${(1 - index / 10).toFixed(1)}`} title={`${(1 - index / 10).toFixed(1)}`} key={`${(1 - index / 10).toFixed(1)}`} />
          ))}
        </Form.Dropdown>
        {/* <Form.TextField title="Webhook" placeholder="Url to send response" {...itemProps.webhookUrl} /> */}
        <Form.Dropdown title="Type communication" placeholder="Type communication assistant" {...itemProps.typeCommunication}>
        ...
        <Form.TextArea title="Additional data" placeholder="Additional data your assistant" {...itemProps.additionalData} />
        <Form.Separator />

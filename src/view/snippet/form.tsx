...
export const SnippetForm = (props: { snippet?: TalkSnippetType; use: { snippets: SnippetHookType }; name?: string }) => {
  const { use, snippet } = props;
  const { pop } = useNavigation();

  const { handleSubmit, itemProps } = useForm<TalkSnippetType>({
    onSubmit: async (snippet) => {
      let updatedItem: TalkSnippetType = { ...snippet };
  ...
  return (
    ...
    >
      ...
      <Form.Dropdown title="Type communication" placeholder="Type communication snippet" {...itemProps.typeCommunication}>

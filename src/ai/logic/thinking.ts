...
export const IntentRecognition = (assistantName: string, question: string) => {
  const messages = [new AIMessage(`As ${assistantName}, describe the user's intent.`), new HumanMessage(question)];
  ...
  const r = await model.invoke([
    new AIMessage(`
      ...
      available actions###${actions
        .map((action) => `(${action[0].metadata.uuid}) + ${action[0].pageContent}`)
        .join("\n\n")}###
    `),
